...
st.sidebar.markdown("---")
st.sidebar.caption(f"DB: {total_matches:,} 試合 | {DB_PATH.name}")
...
with tab1:
    ...
    st.caption("リーグとEV閾値をサイドバーで選択し、実行ボタンを押してください")
    ...
    if st.button("バックテスト実行", key="run_bt", type="primary"):
        try:
            ...
            if result is None or result.total_bets == 0:
                ...
                win_rate = f"{result.wins/result.total_bets*100:.1f}%" if result.total_bets > 0 else "N/A"
                col5.metric("勝率", win_rate)
    ...
    if st.button("シミュレーション実行", key="run_sim", type="primary"):
        try:
            ...
            if result is None or not result.bet_history:
                ...
                st.dataframe(display.sort_values("日付", ascending=False),
